...
from s3t.settings import SENDGRID_KEY, FROM_EMAIL, FROM_NAME


def send_email(subject=None, from_email=None, to_email=None, content=None, from_name=None):
    """Returns the error (string) if there is any, None otherwise"""
    if SENDGRID_KEY is None:
        return 'SENDGRID_KEY is not setted up.'
    if None in (subject, to_email, content):
        return 'subject, to_email, content are required.'
